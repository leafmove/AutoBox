...
AutoBox Agent 的整体架构由以下核心模块组成：

![system.jpg](https://autobox-1326946848.cos.ap-beijing.myqcloud.com/md/system.jpg)
...
| **CLI** | 命令行系统，提供终端命令（`autobox`）和对话命令（`/`），支持进程管理、技能安装、配置修改、知识库管理等操作 |

![gemini_20260422_104418](https://autobox-1326946848.cos.ap-beijing.myqcloud.com/md/gemini_20260422_104418.png)

## Agent 模式

启用 Agent 模式后，AutoBox Agent 会以自主智能体的方式运行，核心工作流如下：
...
Agent 的工作空间默认位于 `~/autobox` 目录，用于存储系统提示词、记忆文件、技能文件等：
...
| `agent_workspace` | 工作空间路径 | `~/autobox` |
